    ...
    <div className="glass-panel chart-panel" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <div style={{ width: '100%', height: '140px', position: 'relative' }}>
        <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: '100%', overflow: 'visible' }}>
          ...
          <line x1="0" y1="10" x2={width} y2="10" stroke="rgba(255,255,255,0.05)" strokeDasharray="4 4" />
          <line x1="0" y1={height / 2} x2={width} y2={height / 2} stroke="rgba(255,255,255,0.05)" strokeDasharray="4 4" />
          <line x1="0" y1={height - 10} x2={width} y2={height - 10} stroke="rgba(255,255,255,0.05)" strokeDasharray="4 4" />

          {/* Paths */}
          <path d={speedPath} fill="none" stroke="#38bdf8" strokeWidth="2.5" strokeLinecap="round" />
          <path d={fuelPath} fill="none" stroke="#10b981" strokeWidth="2" strokeDasharray="3 3" />
          <path d={tempPath} fill="none" stroke="#f43f5e" strokeWidth="2" />

          {/* Points */}
          {points.map((p, i) => (
            <g key={i}>
              <circle cx={getX(i)} cy={getSpeedY(p.speed)} r="3" fill="#38bdf8" />
              <circle cx={getX(i)} cy={getFuelY(p.fuel)} r="2.5" fill="#10b981" />
              <circle cx={getX(i)} cy={getTempY(p.engineTemp)} r="2.5" fill="#f43f5e" />
            </g>
          ))}
        </svg>
      </div>
